...
const val MAIN_CURRENCY = "eur"
interface CryptoApi {
    @GET("coins/markets")
    fun fetchCurrencies(@Query("vs_currency") vsCurrency: String = MAIN_CURRENCY,
    ...
    fun fetchCurrencyDetails(@Query("ids") ids: String,
                             @Query("vs_currency") vsCurrency: String = MAIN_CURRENCY) : Call<List<CurrencyItem>>

    @GET("coins/{id}/market_chart")
    fun fetchCurrencyChartData(@Path("id") id: String,
                                 @Query("vs_currency") vsCurrency: String = MAIN_CURRENCY,
